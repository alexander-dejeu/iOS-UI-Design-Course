...
class ThreeCircles: UIView {
  ...
  private var a = CABasicAnimation(keyPath: "strokeEnd")
  private var b = CABasicAnimation(keyPath: "strokeStart")
  ...
  func endAnimation(){
    ...
    let runningTime = date.timeIntervalSince(startTime as Date)
    
    //So we happen to know the total time is : 2.84 * 1.103
    let runTime = 2.84 * 1.103
    
    let percentage = runningTime.truncatingRemainder(dividingBy: runTime) / runTime * 100.0
